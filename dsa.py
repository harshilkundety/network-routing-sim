...
def welch_pvalue(a_vals, b_vals):
    """
    Two-tailed Welch t-test p-value without scipy.
    Uses the normal approximation, which is accurate for n > 30.
    Returns 1.0 (not significant) if inputs are too small to test.
    """
    n1, n2 = len(a_vals), len(b_vals)
    if n1 < 2 or n2 < 2:
        return 1.0
    m1, m2 = avg(a_vals), avg(b_vals)
    s1, s2 = std(a_vals), std(b_vals)
    se = math.sqrt(s1 * s1 / n1 + s2 * s2 / n2)
    if se == 0:
        return 0.0 if m1 != m2 else 1.0
    t_stat = (m1 - m2) / se
    # Normal approximation — accurate for combined n > 30 (e.g. 30+30 runs)
    p = 2.0 * (1.0 - 0.5 * (1.0 + math.erf(abs(t_stat) / math.sqrt(2.0))))
    return max(0.0, min(1.0, p))

# ─────────────────────────────────────────────
# LATENCY MODEL CONSTANTS
# These define simulation time units only — not real 6G milliseconds.
# Absolute values carry no physical meaning without a proper channel
# model (3GPP NR, TDL-C, etc.). Use them only for relative comparisons.
# ─────────────────────────────────────────────
TICK_MS = 10   # ms per simulation tick (arbitrary, no physical basis)
HOP_MS  = 2    # ms per hop            (arbitrary, no physical basis)
